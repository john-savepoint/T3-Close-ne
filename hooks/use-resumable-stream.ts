...
export function useResumableStream(options: UseResumableStreamOptions = {}) {
  const { autoReconnect = true, maxReconnectAttempts = 3, reconnectDelay = 2000 } = options
  ...
  const connectToStream = useCallback(
    (streamSessionId: string, isReconnection = false) => {
  ...
  const startStream = useCallback(
    async (prompt: string, model: string = "openai/gpt-4o-mini"): Promise<string | null> => {
      try {
        setContent("")
        setError(null)
  ...
  const getStreamInfo = useCallback(
    async (streamSessionId?: string): Promise<StreamSession | null> => {
