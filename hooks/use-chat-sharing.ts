...
export function useChatSharing() {
  ...
  const revokeSharedLink = useCallback(async (chatId: string): Promise<void> => {
    ...
    try {
      ...
      setSharedChats((prev) =>
        prev.map((share) => (share.chatId === chatId ? { ...share, isActive: false } : share))
      )
    } catch (error) {
          ...
          messages: [
            {
              ...
              type: "user",
              content:
                "I'm having trouble with flexbox alignment. My items aren't centering properly.",
  ...
  const incrementViewCount = useCallback(async (token: string): Promise<void> => {
    ...
    setSharedChats((prev) =>
      prev.map((share) =>
        share.token === token ? { ...share, viewCount: (share.viewCount || 0) + 1 } : share
      )
    )
  }, [])
