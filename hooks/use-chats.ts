...
export function useChats(options: UseChatPersistenceOptions = {}) {
  ...
  const stats = useQuery(
    api.chats.getStats,
    options.userId ? { userId: options.userId } : "skip"
  )
  ...
  const moveToArchive = useCallback(async (chatId: Id<"chats">): Promise<void> => {
    try {
  ...
  const moveToTrash = useCallback(async (chatId: Id<"chats">): Promise<void> => {
    try {
      await updateStatusMutation({
        chatId,
        status: "trashed",
  ...
  const restoreChat = useCallback(async (chatId: Id<"chats">): Promise<void> => {
    try {
      await updateStatusMutation({
        chatId,
        status: "active",
  ...
  const deletePermanently = useCallback(async (chatId: Id<"chats">): Promise<void> => {
    try {
  ...
  const duplicateChat = useCallback(async (
    chatId: Id<"chats">,
    newTitle?: string
  ): Promise<Id<"chats"> | null> => {
    if (!options.userId) return null
