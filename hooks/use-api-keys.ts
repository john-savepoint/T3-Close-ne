...
export function useApiKeys() {
    ...
    try {
      ...
      if (stored) {
        ...
        const isValidationFresh =
          data.lastValidated && now - data.lastValidated < VALIDATION_CACHE_TTL
  ...
  const saveToStorage = useCallback(
    (newConfig: ApiKeyConfig, newValidation?: Record<string, ApiKeyValidationResult>) => {
      try {
  ...
  const updateKey = useCallback(
    async (provider: keyof ApiKeyConfig, apiKey: string) => {
  ...
  const validateKey = useCallback(
    async (provider: keyof ApiKeyConfig): Promise<ApiKeyValidationResult> => {
  ...
  const configuredProviders = Object.keys(config).filter(
    (key) => config[key as keyof ApiKeyConfig]
  ) as (keyof ApiKeyConfig)[]
