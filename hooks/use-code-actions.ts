...
export function useCodeActions() {
  ...
  const generateFilename = useCallback(
    (language: string, customFilename?: string): string => {
  ...
  const downloadCode = useCallback(
    (code: string, language: string, filename?: string): void => {
      try {
  ...
  const shareCode = useCallback(
    async (code: string, language: string, title?: string): Promise<boolean> => {
      if (!navigator.share) {
        return false
      }
   ...
   */
  const copyWithLanguage = useCallback(
