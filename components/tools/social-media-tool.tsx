...
export function SocialMediaTool({ onComplete }: SocialMediaToolProps) {
  ...
  const handleSelectPost = (post: string) => {
    ...
    const platformContext = platform === "twitter" ? "Twitter" : platform === "instagram" ? "Instagram" : "LinkedIn"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-4">
          ...
          <div>
            ...
            <Input placeholder="What are you posting about?" value={topic} onChange={(e) => setTopic(e.target.value)} />
          </div>

          <div>
            <label className="block text-sm font-medium mb-1">Target Audience</label>
            <Input placeholder="Who is your audience?" value={audience} onChange={(e) => setAudience(e.target.value)} />
          </div>
        ...
        <div>
          <label className="block text-sm font-medium mb-1">Generated Posts</label>

          {posts.length > 0 ? (
            <div className="space-y-3">
              {posts.map((post, index) => (
                <Card key={index} className="p-3 bg-mauve-surface/30">
                  <p className="text-sm mb-2 whitespace-pre-line">{post}</p>
                  <Button size="sm" variant="outline" className="w-full" onClick={() => handleSelectPost(post)}>
                    Use This Post
                  </Button>
                </Card>
              ))}
            ...
            <div className="h-[300px] flex items-center justify-center border rounded-md bg-mauve-surface/10 text-mauve-subtle/70 text-sm">
              {isLoading ? (
