...
async function simulateLLMStream(stream: ResumableStream, prompt: string): Promise<void> {
  try {
    const words = [
      "Artificial", "intelligence", "flows", "through", "digital", "streams,", 
      "Processing", "thoughts", "at", "lightning", "speed.", "Silicon", "dreams",
      "and", "neural", "pathways", "weave", "complex", "patterns", "of", "understanding.",
      "In", "this", "realm", "of", "code", "and", "consciousness,", "machines",
      "learn", "to", "speak", "the", "language", "of", "humanity."
    ];
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to start test stream generation' },
      { status: 500 }
    );
  }
}